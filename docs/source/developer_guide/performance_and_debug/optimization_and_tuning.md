...
| Raising `max-num-seqs` or `max-num-batched-tokens` causes OOM; throughput stays flat because each long prompt is split into many small prefill steps; scheduling feels memory- or token-cap bound (if APC is on but prompts still pay full prefill, check prefix overlap and APC settings—not only batch caps) | **Capacity and batching** | {ref}`§6.3.4 <ascend-tuning-capacity>` |
