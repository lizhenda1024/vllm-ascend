...
|--------|-----------|-----------|---------------|-----------------|
| **Rope reuse** | Redundant position-encoding work on every layer | `cos`/`sin` cache and index selection run once; later layers reuse the result | **Default** on supported models/paths (often via Npugraph_ex on FULL paths) | N/A for most users |
| [**AddRMSNormQuant fusion**](../../user_guide/configuration/additional_config.md#ascend-compilation-config) | Extra memory traffic around norm + quant | Fuses add, RMSNorm, and quant into fewer kernels | **Default** via `fuse_norm_quant` in `ascend_compilation_config` | Turning off compile passes without a reason |
| **Zero-like elimination** | Useless tensor ops before attention matmuls | Removes zero-like ops from the attention forward path | **Default** on supported compile paths | N/A for most users |
