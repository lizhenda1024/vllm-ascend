...
|--------|-----------|-----------|---------------|-----------------|
| **Rope reuse** | Redundant position-encoding work on every layer | `cos`/`sin` cache and index selection run once; later layers reuse the result | **Default** on supported models | N/A for most users |
| **AddRMSNormQuant fusion** | Extra memory traffic around norm + quant | Fuses add, RMSNorm, and quant into fewer kernels | **Default** via compile passes (`fuse_norm_quant` in `ascend_compilation_config`) | Turning off compile passes without a reason |
| **Zero-like elimination** | Useless tensor ops before attention matmuls | Removes zero-like ops from the attention forward path | **Default** on supported paths | N/A for most users |
| **QKNorm–Rope fusion** | Kernel launch and memory traffic around QK norm + RoPE | Fuses QK norm and RoPE when shapes match (for example, `head_dim == 128`) | **Default** where supported (`fuse_qknorm_rope`); set `false` if Triton is unavailable | Models or dtypes the pass does not support |
| **Graph mode (ACLGraph + Npugraph_ex)** | Per-step launch and Python scheduling overhead | ACLGraph captures and replays the execution graph; Npugraph_ex rewrites the FX graph **before** capture (it does not replace ACLGraph) | `--compilation-config` (for example, `"cudagraph_mode": "FULL_DECODE_ONLY"`) | `enforce_eager=True`; some context-parallel + `FULL` combinations — see [Graph Mode Guide](../../user_guide/feature_guide/graph_mode.md) |
| **`cudagraph_capture_sizes`** | Padding waste when batch token count falls between captured sizes | Pre-captures graphs only for listed token counts; other counts round up to the next size | List sizes in `--compilation-config` after target concurrency is known | Sizes not aligned with real traffic; with FlashComm1, entries should be **multiples of TP** (others are dropped) |
| **`--async-scheduling`** | CPU becomes the limiter at high concurrency | Overlaps scheduling work with NPU execution | CLI flag | Check interaction with speculative decoding, pipeline parallel, and batch-invariant mode |
...
**Do not mix these three options.** They optimize different axes:

| Option | Addresses | Mechanism | How to enable | When not to use |
|--------|-----------|-----------|---------------|-----------------|
| **FlashComm1 (FC1)** | Large AllReduce + RMSNorm (+ quant) phases inside a **TP team** | Custom ops implement **ReduceScatter → local RMSNorm (+ quant) → AllGather** instead of one AllReduce on full hidden states; active only when the scheduler’s **token count is above an internal threshold** | `--additional-config '{"enable_flashcomm1": true}'` | Small batches (below threshold, may be slower than baseline); vision-language models (use SP Pass instead); combinations your model doc marks unsupported |
| **Sequence parallelism (SP, Pass)** | Same collective pattern as FC1, for **VL** graphs | Compile-time pass rewrites AllReduce+RMSNorm to RS/AG; requires graph mode | `--compilation-config '{"pass_config": {"enable_sp": true}}'` | Non-VL models today; quantized SP still limited — [Sequence Parallelism](../../user_guide/feature_guide/sequence_parallelism.md) |
| **Context parallel (PCP / DCP)** | Memory and time on **very long sequences** | Splits sequence dimension across ranks for prefill and/or decode | `--prefill-context-parallel-size`, `--decode-context-parallel-size` | Short-context workloads; orthogonal to FC1 — pick for length, not for TP Norm comm |
| **`enable_matmul_allreduce`** | Matmul immediately followed by AllReduce under TP | **Custom-op** fusion (separate code path from the MatmulAllReduceAddRMSNorm **compiler pass** on some Npugraph_ex graphs) | `--additional-config '{"enable_matmul_allreduce": true}'` | Default **off**; enable only when your model/hardware guide recommends it |
| **FlashComm2** | Memory and comm around **output projection (`o_proj`)** | Shards `o_proj` with `flashcomm2_parallel_size` and adjusts comm domains; not a replacement for FC1 | `--additional-config '{"flashcomm2_parallel_size": N}'` (`N > 0`) | Treating it as “FC1 version 2”; see [layer sharding](../../user_guide/feature_guide/layer_sharding.md) |
| **Fine-grained TP** | Uneven comm across modules (lm_head, MLP, embedding) | Different TP widths per module | `finegrained_tp_config` in `--additional-config` | Before basic TP size is sane |
| **Layer sharding** | Full-layer weights too large (often PD prefill node) | Shards selected linear layers across ranks | `layer_sharding` in `--additional-config` (PD scenarios) | Dense single-node jobs with enough memory |
...
| Option | Addresses | Mechanism | How to enable | When not to use |
|--------|-----------|-----------|---------------|-----------------|
| **Weight prefetch** | MTE stalls before large linear layers | A side pipeline issues **CMO** prefetches into L2 while vector ops (RMSNorm, SwiGLU, etc.) run on another pipeline | `--additional-config` → `weight_prefetch_config` (`enabled`, `prefetch_ratio`) | Latency-sensitive serving; before graph path is stable; MLP `down` prefetch needs SP enabled — [Weight Prefetch](../../user_guide/feature_guide/weight_prefetch.md) |
| **`weight_nz_mode`** | Cube efficiency on weight layout | Stores weights in **FRACTAL_NZ** when allowed (`0` off, `1` quant only, `2` aggressive) | `--additional-config '{"weight_nz_mode": 1}'` (default **1**) | Expecting it to fix HCCL bottlenecks |
| **Quantization (W8A8, W4A8, …)** | Arithmetic and collective volume | Lower precision kernels and smaller transfers | Model-specific quant flags and weights | Without accuracy validation; MoE paths may need extra options (MLAPO, fused MC2) per model guide |
| **`enable_mlapo`** (MoE) | DeepSeek-class W8A8 layer execution | Layer-wise adaptive parallel layout; trades **more NPU memory** for speed | Default **on**; disable via `--additional-config '{"enable_mlapo": false}'` if memory is tighter than latency |
| **`enable_fused_mc2`** (MoE) | MoE dispatch/combine overhead | Replaces default ALLTOALL+MC2 with fused operators under strict constraints | `--additional-config '{"enable_fused_mc2": 1}'` or `2` | Wrong PD role, EP size, or MTP dtype — see [Large-scale EP](../../user_guide/feature_guide/large_scale_ep.md) |
| **EPLB** (MoE) | Hot experts on fixed EP layout | Rebalances expert placement from heat maps | `eplb_config` in `--additional-config` | Small EP jobs where overhead dominates |
...
| Option | Addresses | Mechanism | How to enable | When not to use |
|--------|-----------|-----------|---------------|-----------------|
| **`max-num-batched-tokens`** | Throughput vs peak memory per scheduler step | Caps total tokens in one forward pass | CLI | Too low → many chunked-prefill rounds per long prompt, throughput collapses; too high → OOM |
| **`max-num-seqs`** | Concurrent sequences vs KV footprint | Caps number of active sequences | CLI | Raising it without memory headroom |
| **Chunked prefill** | Long single prompts blocking the batch | Splits prefill into multiple forward passes under the token cap | vLLM scheduler (often on by default) | `max-num-batched-tokens` so small that each chunk is tiny — looks like “low QPS” in §6.2 |
| **Prefix caching (APC)** | Repeated prompt prefixes | Reuses KV blocks for shared prefixes | vLLM prefix-cache settings | Expecting gain when prompts do not share prefixes |
| **PD / EPD disaggregation** | Competing TTFT and TPOT on one pool | Runs prefill and decode on separate engines | `kv_transfer_config`, roles — [disaggregated prefill](../Design_Documents/disaggregated_prefill.md) | Simple single-node latency tests without PD infra |
| **KV offload / KV pool** | KV exceeds device memory | Moves or shares KV across CPU/remote store | Feature-specific setup — [KV offload](../../user_guide/feature_guide/kv_cache_cpu_offload.md), [KV pool](../../user_guide/feature_guide/kv_pool.md) | Before batch limits are tuned |
| **Dynamic batch / balance scheduling** | Uneven load across time or ranks | Adjusts batching from SLO or load signals | `SLO_limits_for_dynamic_batch`, `enable_balance_scheduling` in `--additional-config` | Until baseline batching is stable |
...
5. Bandwidth (quant, weight_nz_mode; weight prefetch only for throughput after profiling)
```
...
Set `cudagraph_capture_sizes` to the concurrency levels you actually run; with FC1 enabled, use multiples of TP.
