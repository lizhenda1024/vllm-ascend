...
**Parallel knobs at a glance:** **TP** reduces per-rank tensor size but adds collectives; **PP** reduces per-rank layer memory but adds stage bubbles; **EP** scales MoE experts but adds dispatch/combine traffic; **PCP/DCP (context parallel)** split long sequences; **PD/EPD** split service roles across instances. Choose by bottleneck type first, then tune inside that category.
...
| [**Fine-grained TP**](../../user_guide/feature_guide/Fine_grained_TP.md) | Uneven comm across modules (lm_head, MLP, embedding) after base topology is fixed | Keeps the same global **TP family**, but assigns different TP widths to selected modules; unlike **PP** (pipeline-stage split) and **EP** (expert routing split), this is intra-model TP reshaping to smooth per-layer comm/computation imbalance | [`finegrained_tp_config`](../../user_guide/configuration/additional_config.md) in `--additional-config` | Before basic TP/PP/EP topology is sane |
...
| [**PD / EPD disaggregation**](../../user_guide/feature_guide/epd_disaggregation.md) | Competing TTFT and TPOT on one pool; multimodal encoder work interfering with language decode | **PD** separates prefill and decode engines. **EPD** (disaggregated encoder) further separates the multimodal vision encoder from P/D instances, with connector-based embedding transfer and optional load-balancing proxy | `kv_transfer_config`, roles; EPD examples in `examples/online_serving/disaggregated_encoder/*`; see also [disaggregated prefill](../Design_Documents/disaggregated_prefill.md) | Simple single-node text-only latency tests without disaggregation infra |
